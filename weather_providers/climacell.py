...
class Climacell(BaseWeatherProvider):
    ...
    def get_icon_from_climacell_weathercode(self, weathercode, is_daytime):
        icon_dict = {
            ...
            1101: "scattered_clouds"
            if is_daytime
            else "partlycloudynight",  # Partly Cloudy
            1102: "mostly_cloudy"
            if is_daytime
            else "mostly_cloudy_night",  # Mostly Cloudy
            ...
            4200: "climacell_rain_light"
            if is_daytime
            else "rain_night_light",  # Light Rain
            4201: "climacell_rain_heavy"
            if is_daytime
            else "rain_night_heavy",  # Heavy Rain
            5000: "snow",  # Snow
        ...
        logging.debug(
            "get_icon_by_weathercode({}, {}) - {}".format(weathercode, is_daytime, icon)
        )
    ...
    def get_description_from_climacell_weathercode(self, weathercode):
        ...
        logging.debug(
            "get_description_by_weathercode({}) - {}".format(weathercode, description)
        )
    ...
    def get_weather(self):
        ...
        url = (
            ...
            + "?location={}&units={}&fields=temperatureMin&fields=temperatureMax&fields=weatherCode&timesteps=1d&apikey={}".format(
                location_latlong, self.units, self.climacell_apikey
            )
        )
        ...
        weathers = []
        for interval in response_data["data"]["timelines"][0]["intervals"]:
            weather_data = interval["values"]
            logging.debug("get_weather() - {}".format(weather_data))
            ...
            weather["icon"] = self.get_icon_from_climacell_weathercode(
                weather_data["weatherCode"], daytime
            )
            weather["description"] = self.get_description_from_climacell_weathercode(
                weather_data["weatherCode"]
            )

            weathers.append(weather)

        logging.debug(weathers)
        return weathers
